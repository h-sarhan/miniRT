...
/*   Updated: 2022/12/20 18:18:10 by hsarhan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	spawn_shape(t_scene *scene)
{
	...
	scene->shapes[scene->count.shape_count].origin.x = \
	scene->camera.orientation.x * 5 + scene->camera.position.x;
	scene->shapes[scene->count.shape_count].origin.y = \
	scene->camera.orientation.y + scene->camera.position.y;
	scene->shapes[scene->count.shape_count].origin.z = \
...
void	toggle_keys_held(int key, t_scene *scene)
{
	if (key == KEY_W)
...
int	set_key_down(int key, t_scene *scene)
{
	...
	if ((key == KEY_1 || key == KEY_2 || key == KEY_3 || key == KEY_4 ||
		key == KEY_5 || key == KEY_6) && scene->edit_mode == true)
	{
